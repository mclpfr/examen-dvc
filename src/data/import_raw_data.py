...
def import_raw(raw_relative_path, 
                    filenames,
                    bucket_folder_url):
    '''import filenames from bucket_folder_url in raw_relative_path'''
    if check_existing_folder(raw_relative_path):
        os.makedirs(raw_relative_path)
    # download all the files
    for filename in filenames :
        input_file = os.path.join(bucket_folder_url,filename)
        output_file = os.path.join(raw_relative_path, filename)
